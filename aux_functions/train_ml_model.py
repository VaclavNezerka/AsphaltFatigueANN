...
def train_ann_lin_cv(kf_index, actual_opt, actual_temp, train_inputs, test_inputs,
    ...
    # Clip predictions
    train_predictions = np.clip(train_predictions, a_min=2000, a_max=2000000)
    test_predictions = np.clip(test_predictions, a_min=2000, a_max=2000000)
    validate_predictions = np.clip(validate_predictions, a_min=2000, a_max=2000000)
...
def train_ann_lin_val_train(actual_opt, actual_temp, train_inputs, train_outputs, validate_inputs, validate_outputs,
                            num_layers, n_epochs, dense, activation_function, optimizer,
                            plot_iterations=True):
    """
    Train a sequential ANN model with all available data as the training dataset.

    Args:
    - actual_opt (str): Option used for directory naming.
    - actual_temp (str): Temperature used for directory naming.
    - train_inputs (pd.DataFrame): Input features for training.
    - train_outputs (pd.DataFrame): Output targets for training.
    - num_layers (int): Number of layers in the ANN model.
    - n_epochs (int): Number of epochs for training.
    - dense (int): Number of dense neurons in each layer.
    - activation_function (str): Activation function for the layers.
    - optimizer (str): Optimizer for training the model.
    - plot_iterations (bool): If True, plots the training progress.

    Returns:
    - Directory name where the outputs are saved.
    - Trained model.
    """

    # Define the model
    model = Sequential()
    model.add(Dense(dense, activation=activation_function, kernel_initializer='he_normal',
    ...
    dir_name = f'output/{actual_opt}/{actual_temp}/{num_layers}layers_{dense}dense_{n_epochs}epochs_{optimizer}_optimizer_{activation_function}_activation'
    os.makedirs(dir_name, exist_ok=True)
    ...
    start_epoch = int(n_epochs / 5)
    model_checkpoint = CustomModelCheckpoint(start_epoch, filepath=f'{dir_name}/best_model.h5', save_best_only=True,
                                             monitor='val_loss', mode='min', verbose=0)
    # Train the model
    history = model.fit(train_inputs, train_outputs, epochs=n_epochs, verbose=1,
                        validation_data=(validate_inputs, validate_outputs),
                        callbacks=[model_checkpoint, epoch_logger], shuffle=True)
    ...
    model_path = f'{dir_name}/final_model.h5'
    model.save(model_path)

    # Plot the training progress if required
    if plot_iterations:
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.plot(history.history['loss'], label="Train")
        plt.plot(history.history['val_loss'], label="Validation")
    ...
    return dir_name, model, train_predictions, train_outputs, validate_predictions, validate_outputs

def train_ann_log_val_train(actual_opt, actual_temp, train_inputs, train_outputs, validate_inputs, validate_outputs,
                            num_layers, n_epochs, dense, activation_function, optimizer,
                            plot_iterations=True):
    """
    Train a sequential ANN model with all available data as the training dataset.
